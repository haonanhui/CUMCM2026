...
## 本地质量补充（用户 2026-09-21 明确要求）

本轮按 writing-guide-2 和 figure-style-2 审读实际成品：可见自然语言零引号，把字句与防御性补充否定逐处处置；同时全文读取 `docs/FIGURES_ZH.md`，查图内多余标题/页脚说明、图下 TeX caption 和最终字体。脚本旧 PASS 不覆盖新增要求；未核验保留未知。internal-v3.1 增加语言与图件操作判据，24 项权重沿用 v3。

评分前全文读取 `docs/paper-quality/SCORING_ANCHORS.md`（internal-v3.1）。先做逐问内容矩阵，再按24项锚点评分；审核分数是否与实质缺陷矛盾。下文上游 PASS 仅指相应工程检查，不能替代论文质量状态；未执行编译或视觉检查的相应项记录 UNVERIFIED，不能仅解释原因就算通过。

同时读取 `docs/PAPER_WRITING_GUIDE.md`，核查论证展开、重复防御性声明和图文解释，证据计入原有 D/E/F 子项。逐句判断科学语义，保留必要否定约束；页数与句式命中次数不机械转换为分数。

全文读取项目根 `docs/PAPER_QUALITY_STANDARD.md`，按 internal-v3.1 逐项核验同一候选 PDF，结果与文本门禁、数值验证、编译分开报告。本地补充优先于下文将章节短、图后解释不足一律视为软警告的默认做法：若实质论证缺失，返回 5writing 或建模/求解阶段，不得交付论文质量 PASS。未看实页不能判视觉通过；联系图只能初筛，密集正文、公式、表格与图注须看可读原尺寸。无完整评审时质量状态为 UNVERIFIED，不能拿工程检查代替。
...
- 按项目根 `docs/FIGURES_ZH.md` 检查所有图内文字为中文（公式符号除外），涵盖坐标轴、图例、色条、类别/刻度、子图、节点和注释。残留英文模板说明须返修；数学变量、数字和单位符号不误判。
