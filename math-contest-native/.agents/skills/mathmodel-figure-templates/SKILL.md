...
> 原生工作区适配：执行前读取项目根 docs/HARNESS.md；工具名按当前 Harness 映射，路径按当前项目解析。研究步骤和报告约定保留上游规范。

# MathModel Figure Templates

This skill is bundled into the LaTeX sandbox at `.agents/skills/mathmodel-figure-templates`. It contains ready-to-run Python/matplotlib scripts for the figure templates exposed in the MathModel Improve tab.

## Fast Path

### 本工作区定制：图内中文

交付前必须读取项目根 `docs/FIGURES_ZH.md`。所有图内文字使用中文，公式、变量和单位符号保持数学规范。上游模板脚本含英文标签：运行后只算中间图，必须在复制到工作区的脚本中逐项替换坐标轴、图例、色条、类别/刻度、子图和文字注释，配置真实中文字体，重新导出并实际查看。不能直接把英文模板图当作最终结果交付。

1. Match the requested chart in `references/figure-catalog.md`.
2. From `<project-root>`, run the renderer with the template id:

```bash
python3 .agents/skills/mathmodel-figure-templates/scripts/render_template.py paired-raincloud
```

3. The renderer copies the bundled template script into `绘图复刻/scripts/`, runs it there, and writes outputs to `绘图复刻/outputs/`.
4. Localize visible labels in the copied script according to `docs/FIGURES_ZH.md`, re-render, inspect Chinese text and mathematical symbols, then return the PNG/PDF/SVG and copied script paths.

Use `--list` to show supported ids:

```bash
python3 .agents/skills/mathmodel-figure-templates/scripts/render_template.py --list
```
...
- PNG/PDF/SVG export.
- Chinese labels, legends, annotations and category text, preserving mathematical symbols; verify glyph coverage and high-DPI output.
