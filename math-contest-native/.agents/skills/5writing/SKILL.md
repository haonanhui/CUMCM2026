...
本轮 writing-guide-2 必须落实到成品：可见自然语言零引号；把字句逐处改为具体对象和动作；补充性否定改为直接陈述条件与结果。优势通过实际对照展开，科学条件保持准确。绘图全文读取 `docs/FIGURES_ZH.md` 的 figure-style-2，图题与说明放在图下 TeX caption，图后正文解释机制，按最终尺寸核验字体。

生成正文先全文读取 `docs/PAPER_WRITING_GUIDE.md`（writing-guide-2），按正向论证单元和本题内容计划组织初稿，使用其中语言示例并记录实际读取版本。随后读取下述评分规范；审计记录与论文正文各自使用适合的表达。

执行本阶段前全文读取项目根 `docs/PAPER_QUALITY_STANDARD.md`，按其中“写作前与交付前”执行。保留上游流程，不依赖 app。先完成各问论证清单再写正文，写完后用同一规范自查。参考 `docs/paper-quality/language-patterns.jsonl` 的表达功能与条件，不能把模板当作已成立结论。不能只在最终评分时才读规范。
...
图外图注与图内文字分开处理：本工作区用户要求图内所有说明文字为中文，公式符号除外。选取或嵌入图件前按项目根 `docs/FIGURES_ZH.md` 核验；发现英文模板标签，交回绘图脚本修正后重新导出，不在排版时掩盖。
